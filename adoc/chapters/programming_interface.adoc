...
# The following sentence needs to be fixed.  The submit function doesn't
# "schedule" and it works synchronously.
The [code]#submit#
...
# Add some clarification about the state of "hostPointer" memory
# if set_final_data is called.
...
# T needs to be device copyable.
# We can probably remove the restriction on forward-declarable given the new
...
// TODO: Specify behavior when the same spec constant is set in more than one
