...
to access the contents of a buffer through the OpenCL device’s constant memory.
...
// TODO: How does make_kernel_bundle() know which kernel_id's it contains?  Propose
// that get_kernel_ids() should return an empty list.
...
TODO: What if the application creates a kernel bundle with input state from
  an underlying OpenCL program, and then calls OpenCL "build" on that program?
  This would change the state of the underlying program, but the SYCL object
  would still have state "input".  I think we need to say that operations like
  this are invalid.
...
// TODO: Add a note stating that cl_long (etc.) are not necessarily the same
// as long.
