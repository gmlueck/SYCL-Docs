...
[[command]]command::
    A request to execute work in a <<command-group>> such as the invocation of
    a <<sycl-kernel-function>>, the invocation of a <<host-task>> or an
    asynchronous copy.

[[command-group]]command group::
    A command group represents work that is scheduled for execution at some
    future time along with dependency information which triggers execution of
    that work.  The work contained by a command group, a <<command>>, executes
    when the dependencies in the command group have been satisfied.  A command
    group is created by submitting a <<command-group-function-object>> to a
    <<queue>>.

[[command-group-function-object]]command group function object::
    An object which is callable with [code]#operator()# that takes a
    reference to a <<handler>>.  When a command group function object is
    submitted to a <<queue>>, a <<command-group>> is scheduled for execution
    at some future time.  The function object can be a named type, lambda
    function or [code]#std::function#.
